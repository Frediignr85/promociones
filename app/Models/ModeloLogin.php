...
class ModeloLogin extends Model
{
    function datos_empresa($id_empresa){
        $data = $this->db->query("SELECT * FROM tblsucursal WHERE id_sucursal = '$id_empresa'");
        return $data;
    }
    function verificar_username($username){
        $data = $this->db->query("SELECT * FROM tblusuario WHERE usuario = '$username'");
    ...
    function verificar_credenciales($username,$password){
        ...
        //echo "SELECT * FROM tblusuario WHERE password = '".MD5($password)."' AND usuario = '$username' AND activo = 1";
        $data = $this->db->query("SELECT * FROM tblusuario WHERE password = '".MD5($password)."' AND usuario = '$username' AND activo = 1");
        return $data;
    }
